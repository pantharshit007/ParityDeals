...
--> statement-breakpoint
CREATE TABLE "user_supscriptions" (
	"id" uuid PRIMARY KEY DEFAULT gen_random_uuid() NOT NULL,
	...
	"created_at" timestamp with time zone DEFAULT now(),
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "user_supscriptions_clerk_user_id_unique" UNIQUE("clerk_user_id")
);
--> statement-breakpoint
...
CREATE INDEX "user_supscriptions.clerk_user_id_index" ON "user_supscriptions" USING btree ("clerk_user_id");--> statement-breakpoint
CREATE INDEX "user_supscriptions.stripe_customer_id_index" ON "user_supscriptions" USING btree ("stripe_customer_id");